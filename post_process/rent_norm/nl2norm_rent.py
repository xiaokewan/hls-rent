...
class Hypergraph:
    ...
    def add_blocks_info(self, block_index, block_info):
        self.block_info.append((block_index, block_info))
...
def bipartition(hg, rent_data, hmetis_path, partition_level=0):
    ...
    if len(rent_data) >= partition_level + 1:
        rent_data[partition_level].append([blocks, pins, pin_count_dict])
    else:
        rent_data.append([[blocks, pins, pin_count_dict]])
    if blocks > 2:
...
if __name__ == '__main__':
    ...
    if len(arguments) < 2 or len(arguments) > 3:
        print("Usage: python3 nl2norm_rent.py <blif_file_path> [<output_folder>] <hmetis_path>")
        sys.exit(1)
